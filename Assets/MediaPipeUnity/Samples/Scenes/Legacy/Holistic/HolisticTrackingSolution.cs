...
namespace Mediapipe.Unity.Sample.Holistic
{
  public class HolisticTrackingSolution : LegacySolutionRunner<HolisticTrackingGraph>
  {
    ...
    protected override IEnumerator Run()
    {
      ...
      while (true)
      {
        ...
        if (runningMode.IsSynchronous())
        {
          ...
          // // 🎯 同步模式：调用骨骼映射器
          // if (boneMapper != null && result.poseWorldLandmarks != null){
          //   boneMapper.UpdateBoneMapping(result.poseWorldLandmarks);
          // }

          // 🎯 同步模式：调用 VNectModel 适配器
          if (vnectAdapter != null && result.poseWorldLandmarks != null)
          {
            vnectAdapter.UpdateFromHolistic(result.poseWorldLandmarks);
          }
    ...
    private void OnPoseWorldLandmarksOutput(object stream, OutputStream<LandmarkList>.OutputEventArgs eventArgs)
    {
      ...
      // // 🎯 调用骨骼映射器更新Unity角色骨骼
      // if (boneMapper != null && value != null){
      //   boneMapper.UpdateBoneMapping(value);
      // }
      // 🎯 调用 VNectModel 适配器更新 Unity 角色骨骼
      if (vnectAdapter != null && value != null)
      {
        vnectAdapter.UpdateFromHolistic(value);
      }
    }
